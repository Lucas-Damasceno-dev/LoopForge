...
CURATED_BENCHMARK_PROBLEMS: list[BenchmarkProblem] = [
    ...
    BenchmarkProblem(
        ...
        stack="python",
        idea="Implement a TokenBucket rate limiter class in Python with capacity and refill_rate per second.",
        difficulty="medium",
        baseline_elo=1250.0,
        user_stories=[{"id": "US-011", "title": "Rate Limiter", "description": "Token bucket algorithm."}],
    ),
    BenchmarkProblem(
        id="P-012",
        title="Async Event Emitter & PubSub",
        stack="javascript",
        idea="Implement a lightweight EventEmitter in Node.js supporting on(), off(), emit(), and once() methods.",
        difficulty="medium",
        baseline_elo=1250.0,
        user_stories=[{"id": "US-012", "title": "Event Emitter", "description": "In-memory PubSub system."}],
    ),
    BenchmarkProblem(
        id="P-013",
        title="Thread-Safe Connection Pool",
        stack="java",
        idea="Implement a mock ConnectionPool manager in Java with maxConnections and acquire/release timeout.",
        difficulty="hard",
        baseline_elo=1300.0,
        user_stories=[{"id": "US-013", "title": "Connection Pool", "description": "Resource pool manager in Java."}],
    ),
    BenchmarkProblem(
        id="P-014",
        title="Exponential Backoff Retry Strategy",
        stack="go",
        idea="Implement a RetryWithBackoff function in Go accepting maxAttempts, initialDelay, and a execution closure.",
        difficulty="medium",
        baseline_elo=1250.0,
        user_stories=[{"id": "US-014", "title": "Retry Mechanism", "description": "Exponential backoff retry utility."}],
    ),
    BenchmarkProblem(
        id="P-015",
        title="Zero-Copy Binary Serializer",
        stack="rust",
        idea="Implement a binary serialization buffer in Rust encoding integer headers and byte payloads.",
        difficulty="hard",
        baseline_elo=1350.0,
        user_stories=[{"id": "US-015", "title": "Binary Encoder", "description": "Compact byte serializer in Rust."}],
    ),
]
