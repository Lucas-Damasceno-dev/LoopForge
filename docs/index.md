...
- **Pipeline Autônomo de 9 Agentes**: Ciclo completo de governança (`CPO` $\rightarrow$ `PM` $\rightarrow$ `Tech Lead` $\rightarrow$ `Developer` $\rightarrow$ `QA` $\rightarrow$ `AppSec` + `DevOps` em paralelo $\rightarrow$ `Lessons`).
...
- **Routing Adaptativo**: 4 modos de roteamento — `full` (ciclo completo), `fast` (Developer→QA→Audit para bugfix), `review-only` (QA→Audit), `explore` (Tech Lead spike).
