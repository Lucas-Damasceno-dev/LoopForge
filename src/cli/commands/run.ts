...
import { interactivePlanHelper } from "./plan.js";

export async function runCommand(
  targetDir: string = ".",
  options: { createPr?: boolean; review?: boolean; auto?: boolean; plan?: boolean; skipPlan?: boolean; watch?: boolean; format?: "json" | "text" } = {}
): Promise<void> {
  const resolvedDir = path.resolve(targetDir);

  if ((options.plan || (!options.auto && !options.skipPlan)) && options.format !== "json") {
    await interactivePlanHelper(resolvedDir);
  }
