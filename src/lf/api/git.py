...
class GitPublishPrRequest(BaseModel):
    title: str | None = Field(None, description="Título do PR")
    body: str | None = Field(None, description="Descrição do PR")
    branch_name: str | None = Field(None, description="Nome da branch de destino")
    labels: list[str] = Field(default_factory=lambda: ["loopforge-generated", "automated"])


class GitPublishPrResponse(BaseModel):
    success: bool
    message: str
...
@git_router.post("/{run_id}/publish-pr", response_model=GitPublishPrResponse)
async def publish_pr(run_id: str, payload: GitPublishPrRequest | None = None) -> GitPublishPrResponse:
    """Publica alterações da run como Pull Request no GitHub."""
    repo_dir = _resolve_run_dir(run_id)
    if repo_dir is None:
        raise HTTPException(
            status_code=404,
    ...
    title = (payload and payload.title) or f"feat: LoopForge automated run {run_id[:8]}"
    body = (payload and payload.body) or f"Automated feature implementation generated by LoopForge run {run_id}."
    labels = (payload and payload.labels) or ["loopforge-generated"]

    try:
        from lf.runner.git.pr import create_github_pr
        import subprocess

        # Garante commit das alterações antes de abrir PR
        ...
        success = create_github_pr(title=title, body=body, labels=labels, repo_path=repo_dir)
        if success:
            return GitPublishPrResponse(
                success=True,
                message="Pull Request criado com sucesso no GitHub.",
                branch=branch,
            )
        return GitPublishPrResponse(
            success=False,
            message="Comando gh pr create falhou ou CLI gh não está autenticada.",
            branch=branch,
        )
    except Exception as exc:
        return GitPublishPrResponse(
            success=False,
            message=f"Falha ao publicar PR: {exc}",
            branch="unknown",
        )
