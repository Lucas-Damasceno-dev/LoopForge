...
from lf.config.schema import PlanSchema
...
@click.option(
    "--vision", "-v", prompt="Visão do projeto / Especificação", help="Visão geral ou especificação do projeto"
)
@click.option(
    "--mode",
    "-m",
    ...
    default="full",
    help="Modo de roteamento: full (hierárquico completo) ou fast (caminho rápido)",
)
@click.option("--interactive/--no-interactive", default=True, help="Modo interativo com Spec Review Gate")
def plan_cmd(vision: str, mode: Literal["full", "fast"], interactive: bool):
    ...
    current_mode: Literal["full", "fast"] = mode
    ...
    while True:
        ...
        table = Table(
            title="[bold cyan]DAG de Tarefas Planejadas[/bold cyan]", show_header=True, header_style="bold magenta"
        )
        ...
        mode_badge = (
            "[bold green]FULL-PATH (Hierárquico: CPO → PM → Tech Lead → Dev → QA)[/bold green]"
            if current_mode == "full"
            else "[bold yellow]FAST-PATH (Caminho Rápido: Dev → QA)[/bold yellow]"
        )
        ...
        console.clear()
        console.print(
            Panel(panel_content, title="[bold white]Spec Review Gate — LoopForge v6[/bold white]", border_style="cyan")
        )
        console.print(table)

        if not interactive or not sys.stdin.isatty():
            cfg.plan = PlanSchema(tasks=plan.tasks, graph=plan.graph)
            save_config(cfg)
