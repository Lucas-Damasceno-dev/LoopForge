...
class MCPRegistry:
    ...
    def _server(self, name: str) -> MCPClient:
        if name not in self._clients:
            raise MCPUnavailableError(f"Servidor MCP {name} não disponível")
    ...
    async def list_tools(self, server: str) -> list[dict]:
        if self._status.get(server) != "connected":
            raise MCPUnavailableError(f"Servidor MCP {server} não conectado")
    ...
    async def call_tool(self, server: str, name: str, args: dict[str, Any]) -> Any:
        ...
        if spec is None or name not in spec.tools_allowlist:
            raise MCPPermissionError(f"Tool {server}:{name} não permitida (allowlist do ade.yaml)")
        if self._status.get(server) != "connected":
            raise MCPUnavailableError(f"Servidor MCP {server} não conectado")
        try:
            ...
            raise MCPUnavailableError(f"Falha ao chamar {server}:{name}") from exc
