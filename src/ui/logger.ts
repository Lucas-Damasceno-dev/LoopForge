...
export function logIterationStart(iteration: number, strategy: string): void {
  console.log("\n" + chalk.bold.blue("─".repeat(60)));
  console.log(chalk.bold.cyan(`🔄 LoopForge Iteração #${iteration}`) + chalk.gray(` [Estratégia: ${strategy}]`));
...
export function logIterationReport(report: IterationReport): void {
  ...
  if (report.passed) {
    console.log(chalk.bold.green(`✔ Iteração #${report.iteration} PASSOU em todos os runners (${summary.passedCount}/${summary.totalRunners} OK em ${summary.totalDurationMs}ms)`));
  } else {
